...
APP_NAME="relate"
APP_VERSION="0.1.0"
if [ -n "${RELATE_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$RELATE_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${RELATE_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RELATE_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/relateby/relate-cli/releases/download/v0.1.0"
elif [ -n "${RELATE_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$RELATE_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/relateby/relate-cli/releases/download/v0.1.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/relateby/relate-cli/releases/download/v0.1.0"
fi
if [ -n "${RELATE_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$RELATE_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${RELATE_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$RELATE_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${RELATE_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$RELATE_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${RELATE_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${RELATE_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${RELATE_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.31.0"},"source":{"app_name":"relate","name":"relate-cli","owner":"relateby","release_type":"github"},"version":"0.1.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/relate"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/relate"
fi
...
The installer for relate 0.1.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/relateby/relate-cli/releases/download/v0.1.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set RELATE_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "relate-aarch64-apple-darwin.tar.xz")
            _arch="aarch64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="bf1ccfdabe068117110cb8db0195e5f2e9579ae957103fe4c75666fb356e79cd"
            _bins="relate"
            _bins_js_array='"relate"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            ...
            _updater_bin=""
            ;;
        "relate-aarch64-unknown-linux-gnu.tar.xz")
            _arch="aarch64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="566a1ba41f1ae8ff190f168e84c7c5d8b36f3bc41ed0850e606f2465c7abbca4"
            _bins="relate"
            _bins_js_array='"relate"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "relate-x86_64-apple-darwin.tar.xz")
            _arch="x86_64-apple-darwin"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="be68341fa8b220ebf0e486e3d5ee48c003040140a2837b6a0f2ae7d347402d84"
            _bins="relate"
            _bins_js_array='"relate"'
            _libs=""
            _libs_js_array=""
            ...
            _updater_bin=""
            ;;
        "relate-x86_64-pc-windows-msvc.zip")
            _arch="x86_64-pc-windows-msvc"
            _zip_ext=".zip"
            _checksum_style="sha256"
            _checksum_value="7e64333d87cd748df7da14e0b085969042ca5cd9768fe23a6c6278c3dc5c601d"
            _bins="relate.exe"
            _bins_js_array='"relate.exe"'
            _libs=""
            _libs_js_array=""
            _staticlibs=""
            _staticlibs_js_array=""
            ...
            _updater_bin=""
            ;;
        "relate-x86_64-unknown-linux-gnu.tar.xz")
            _arch="x86_64-unknown-linux-gnu"
            _zip_ext=".tar.xz"
            _checksum_style="sha256"
            _checksum_value="7515a02c00b395d30c315b5c82bd97bcdb641f2ad6fda69d388b32a0afc99552"
            _bins="relate"
            _bins_js_array='"relate"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="relate-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="relate-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            _archive="relate-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="relate-aarch64-unknown-linux-gnu.tar.xz"
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="relate-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "35"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${RELATE_INSTALL_DIR:-}" ]; then
        _force_install_dir="$RELATE_INSTALL_DIR"
