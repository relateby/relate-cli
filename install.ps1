...
$app_name = 'relate'
$app_version = '0.1.0'
if ($env:RELATE_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:RELATE_DOWNLOAD_URL)
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrls = @($env:INSTALLER_DOWNLOAD_URL)
} elseif ($env:RELATE_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:RELATE_INSTALLER_GHE_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/relateby/relate-cli/releases/download/v0.1.0")
} elseif ($env:RELATE_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:RELATE_INSTALLER_GITHUB_BASE_URL
  $ArtifactDownloadUrls = @("$installer_base_url/relateby/relate-cli/releases/download/v0.1.0")
} else {
  $ArtifactDownloadUrls = @("https://github.com/relateby/relate-cli/releases/download/v0.1.0")
}

$auth_token = $env:RELATE_GITHUB_TOKEN
...
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\relate"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\relate"
}

if ($env:RELATE_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set RELATE_NO_MODIFY_PATH=1 in the environment"
}

if ($env:RELATE_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:RELATE_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-msvc" = @{
      "artifact_name" = "relate-x86_64-pc-windows-msvc.zip"
      "bins" = @("relate.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Download($download_url, $platforms, $arch) {
  ...
  if (($null -ne $info["updater"]) -and $install_updater) {
    ...
    $out_name = "$tmp\relate-update.exe"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:RELATE_INSTALL_DIR)) {
    $force_install_dir = $env:RELATE_INSTALL_DIR
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/relate-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
